...
static __I uint8_t PLL2MuxTable[16] = {25, 125, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 200};
...
{
    uint32_t tmp = 0, pllmull = 0, pllsource = 0,pll2mull = 0,pll2pred = 0, presc = 0, Pll_6_5 = 0;
    ...
    switch(tmp)
    {
        ...
        case 0x08:
            ...
            pll2mull = (RCC->CFGR2 & CFGR2_PLL2MUL)>>8;
            pll2pred = (RCC->CFGR2 & CFGR2_PREDIV2)>>4;
            ...
            if(RCC->CFGR2 & (1<<16))
            {
                tmp = HSE_VALUE * PLL2MuxTable[pll2mull]/10/(pll2pred+1);
                tmp = pllmull*tmp;
                RCC_Clocks->SYSCLK_Frequency=tmp/((RCC->CFGR2&0x0000000f)+1);
            }
